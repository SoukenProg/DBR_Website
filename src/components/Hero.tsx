...
export function Hero({dbrWork, soukenWork}: { dbrWork?: Work; soukenWork?: Work }) {
    ...
    return (<section className="relative py-20 overflow-hidden">
        ...
        <div className="container text-center">
            ...
            <p className="mt-4 text-white/70">D.B.R. — Dual Blades Residence</p>
            <div className="mt-10 grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
                {/* System D.B.R. 最新作 */}
                {dbrWork && (
                    <div className="flex flex-col items-center gap-4 p-4 rounded-xl border border-accentRed/30 bg-linear-to-br from-accentGreen/10 via-accentBlue/10 to-accentRed/10 shadow-glow">
                        <div className="text-sm font-semibold text-accentRed">System D.B.R. 最新作</div>
                        {getJacketUrl(dbrWork.jacket) ? (
                            <Image src={getJacketUrl(dbrWork.jacket)!} alt={dbrWork.title} width={192} height={192} className="object-cover rounded"/>
                        ) : null}
                ...
                {/* Souken521 最新作 */}
                {soukenWork && (
                    <div className="flex flex-col items-center gap-4 p-4 rounded-xl border border-accentPurple/30 bg-linear-to-br from-accentPurple/10 to-accentBlue/10 shadow-glow">
                        <div className="text-sm font-semibold text-accentPurple">Souken521 最新作</div>
                        {getJacketUrl(soukenWork.jacket) ? (
                            <Image src={getJacketUrl(soukenWork.jacket)!} alt={soukenWork.title} width={192} height={192} className="object-cover rounded"/>
                        ) : null}
